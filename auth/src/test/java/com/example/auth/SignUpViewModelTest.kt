...
class SignUpViewModelTest {
    ...
    fun `on event SendOtp on success change screen to VerifyPhoneStage` () = runTest {
        ...
        viewModel.onEvent(SignUpEvent.SendOtp(AuthConstants.SUCCESS_PHONE_NO))
        testDispatcher.scheduler.advanceUntilIdle()
        assert(viewModel.state.phoneNo == AuthConstants.SUCCESS_PHONE_NO)
...
//        assert(viewModel.state.error == null)
        viewModel.onEvent(SignUpEvent.SendOtp(AuthConstants.FAIL_PHONE_NO))
        testDispatcher.scheduler.advanceUntilIdle()
        assert(viewModel.state.phoneNo == AuthConstants.FAIL_PHONE_NO)
...
//        viewModel.state.screen = SignUpProgress.VerifyPhoneStage
        viewModel.onEvent(SignUpEvent.WrongNo)
...
//        viewModel.state.phoneNo = Constants.SUCCESS_PHONE_NO
        //action
        viewModel.onEvent(SignUpEvent.VerifyClick(AuthConstants.VERIFY_OTP_SUCCESS, 15))
        testDispatcher.scheduler.advanceUntilIdle()
        //assert
...
//        viewModel.state.phoneNo = Constants.SUCCESS_PHONE_NO
        //action
        viewModel.onEvent(SignUpEvent.VerifyClick(AuthConstants.VERIFY_OTP_INCORRECT, 15))
        testDispatcher.scheduler.advanceUntilIdle()
        //assert
        assert(viewModel.state.screen == SignUpProgress.VerifyPhoneStage)
